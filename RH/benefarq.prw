...
Private cSelect	:= ""
Private cFrom		:= ""
Private lChkBanco     :=  AllTrim(Upper(TcGetDb())) == 'POSTGRES'
Private cForn410 := {}
...
/*If Empty( cArqIni := Alltrim(FTABELA("S018", nLinha, 08)) ) .and. cFornecedor<>"019"
...
If Empty( cArqOut := Alltrim(FTABELA("S018", nLinha, 09)) )
	...
	Return
EndIf
/*If Empty( cFuncVal := Alltrim(FTABELA("S018", nLinha, 10)) ) .and. cFornecedor<>"019"
...
/*If !file( cArqIni ) .and. cFornecedor<>"019"
...
if  MsgYesNo("Marcar como enviados ? (Caso confirme, em caso de recálculo,não serão excluídos )","Atenção")
	// Posiciona Regitro
	dbSelectArea("QD03VB")
	QD03VB->(DbGoTop())
...
//Processa layout fornecedor 019 tabela s018
Static Function Alelo(oProcess)
local cQuery
local nLinha
Local cLinDet
nLinha  := FPOSTAB("S018", Alltrim(cFornecedor), "==", 4)
...
fWrite( nHdl, "%;"+Alltrim(FTABELA("S018", nLinha, 06))+";%;;;;;;;"+CRLF )
//Detalhe
cLinDet :=";NOME DO USUÁRIO;	CPF;	DATA DE NASCIMENTO;	CÓDIGO DE SEXO;	VALOR;	TIPO DE LOCAL ENTREGA;	LOCAL DE ENTREGA;	MATRÍCULA"+CRLF
fWrite( nHdl, cLinDet )
//Query
//-Ajusta o conteudo da cTiposSel pra usar na Query
cQryAux := ""
...
cSelect:= " SELECT M0_CGC,R0_FILIAL,RA_NOME,RA_CIC,RA_NASC,RA_SEXO,R0_MAT,R0_TPVALE, "
cSelect+= " SUM(CASE WHEN RFO_DIAFIX=0 THEN R0_QDIACAL ELSE RFO_DIAFIX END) AS R0_QDIACAL,"
cSelect+= " SUM(CASE WHEN RFO_DIAFIX=0 THEN R0_VALCAL ELSE R0_VLRVALE*RFO_DIAFIX END) AS R0_VALCAL,MAX(R0_VLRVALE) AS R0_VLRVALE,SUM(R0_VLRFUNC) AS R0_VLRFUNC,SUM(R0_VLREMP) AS R0_VLREMP"

cFrom  := " FROM " + RetSqlName("SR0") + " SR0 "
cFrom  += " INNER JOIN " + RetSqlName("SRA") + " SRA "
cFrom  += " ON SRA.RA_MAT = SR0.R0_MAT AND SRA.RA_FILIAL = SR0.R0_FILIAL and SRA.D_E_L_E_T_ = ' ' "
cFrom  += " INNER JOIN " + RetSqlName("SM0") + " SM0 "
cFrom  += " ON SM0.M0_CODFIL = SR0.R0_FILIAL AND SM0.D_E_L_E_T_ = ' ' "
cFrom  += " INNER JOIN " + RetSqlName("RFO") + " RFO ON RFO.RFO_TPBEN = SR0.R0_TPBEN and RFO.RFO_CODIGO=SR0.R0_CODIGO and RFO.D_E_L_E_T_ = ' ' "
cFrom  += " AND rtrim(RFO.RFO_FILIAL) = substr(SR0.R0_FILIAL,1,4) "
cFrom  += " WHERE "
If TcSrvType() != "AS/400"
	cFrom += " SR0.D_E_L_E_T_ = ' ' AND SRA.D_E_L_E_T_ = ' ' "
Else
...
cFrom  += " GROUP BY M0_CGC,R0_FILIAL,RA_NOME,RA_CIC,RA_NASC,RA_SEXO,R0_MAT,R0_TPVALE"
cFrom  += " ORDER BY M0_CGC,R0_FILIAL,RA_NOME,RA_CIC,RA_NASC,RA_SEXO,R0_MAT,R0_TPVALE"
cQryFech += " ORDER BY M0_CGC,R0_FILIAL,RA_NOME,RA_CIC,RA_NASC,RA_SEXO,R0_MAT,R0_TPVALE"
...
While QD03VB->(!Eof())
	...
	nTotRegTP3 += 1
	nSeq += 1
	//JJS 08/06/21 TROCADO , POR ; A DESPEITO DA INFORMAÇÃO DO SITE
	fWrite( nHdl, "%;"+QD03VB->RA_NOME+";"+QD03VB->RA_CIC+";"+dtoc(QD03VB->RA_NASC)+";"+QD03VB->RA_SEXO+";"+alltrim(str(QD03VB->R0_VALCAL))+iif(QD03VB->R0_FILIAL='010101',";PT;",";FI;")+Right(alltrim(QD03VB->M0_CGC),3)+";"+QD03VB->R0_MAT+";%"+CRLF )

	nLin += 1	//-Indica que pode imprimir o Relatorio Final
	nVlr := QD03VB->R0_VALCAL //VALOR TOTAL = FUNC+EMP
	//Totaliza Registros
	nTotal += nVlr
	//IncProc("Processando...")
	QD03VB->(dbSkip())
...
//Processa layout fornecedor 019 tabela s018
Static Function VTRVEM(oProcess)
local cQuery
local nLinha
Local cLinDet
nLinha  := FPOSTAB("S018", Alltrim(cFornecedor), "==", 4)
...
cFrom  += " ON SRA.RA_MAT = SR0.R0_MAT AND SRA.RA_FILIAL = SR0.R0_FILIAL "
cFrom  += " INNER JOIN " + RetSqlName("SM0") + " SM0 "
cFrom  += " ON SM0.M0_CODFIL = SR0.R0_FILIAL "
cFrom  += " INNER JOIN " + RetSqlName("RFO") + " RFO ON RFO.RFO_TPBEN = SR0.R0_TPBEN "
cFrom  += " AND rtrim(RFO.RFO_FILIAL) = substr(SR0.R0_FILIAL,1,4) "
cFrom  += " WHERE "
If TcSrvType() != "AS/400"
	cFrom += " SR0.D_E_L_E_T_ = ' ' AND SRA.D_E_L_E_T_ = ' ' "
...
While QD03VB->(!Eof())
	...
	nTotRegTP3 += 1
	nSeq += 1
	//fWrite( nHdl, "%,"+QD03VB->RA_NOME+","+QD03VB->RA_CIC+","+dtoc(QD03VB->RA_NASC)+","+QD03VB->RA_SEXO+","+alltrim(str(QD03VB->R0_VALCAL))+",FI,"+QD03VB->M0_CGC+","+QD03VB->R0_MAT+",%"+CRLF )

	if QD03VB->R0_VALCAL =0
		QD03VB->(dbSkip())
		loop
	endif
	cValDia:=""
	if 	QD03VB->R0_QDIACAL>0
		nValDia :=QD03VB->R0_VALCAL/QD03VB->R0_QDIACAL
		cValDia :=alltrim(str(NoRound(nValDia*100,0)))
	endif
	fWrite( nHdl, QD03VB->RA_CIC+"|"+alltrim(str(QD03VB->R0_QDIACAL))+"|"+cValDia+"|"+QD03VB->RA_NOME+CRLF )
